...
const columns = [
  ...
  { id: "brillo", label: "Brillo Producto 1", minWidth: 170 },
  { id: "brillo_producto1", label: "Código Producto 1", minWidth: 170 },
  { id: "brillo2", label: "Brillo Producto 2", minWidth: 170 },
  { id: "termi_brillo2", label: "Código Producto 2", minWidth: 170 },
...
export default function LimpiezaExtrucion() {
  ...
  useEffect(() => {
    const fetchExistingDates = async () => {
      try {
        const response = await fetchBrilloLimpieza();
  ...
  return (
    <div>
      <h3 style={{ fontSize: "20px" }}>Tiempos de Limpieza Brillo</h3>
                    ...
                    item.producto2.map((producto, index) => (
                      ...
                      >
                        {index === 0 && (
                          <>
                            <TableCell
                              className={getColorClass(item.id, "brillo-cell")}
