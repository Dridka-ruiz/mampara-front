...
import { apiUrlEtiquetasMol8 } from "../../../../api/molinos/apiMol8";
...
const EtiquetasMol8 = ({ etiquetasMol8, setEtiquetasMol8, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol8);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol8),
          fetchProductos(),
        ]);
        setEtiquetasMol8(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol8.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol8(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol8);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol8", etiquetasMol8);
    }
  }, [etiquetasMol8, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol8, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol8.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol8.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol8.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetasMol8.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
