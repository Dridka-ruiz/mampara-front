...
import { apiUrlEtiquetasMol17 } from "../../../../api/molinos/apiMol17";
...
const EtiquetasMol17 = ({ etiquetasMol17, setEtiquetasMol17, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol17);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol17),
          fetchProductos(),
        ]);
        setEtiquetasMol17(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol17.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol17(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol17);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol17", etiquetasMol17);
    }
  }, [etiquetasMol17, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol17, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol17.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol17.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol17.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetasMol17.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
      ...
      <PermisoValidator permiso="molinos.mover">
        <div className="position etiquetasAgregadas">
          <div className="text-center tittleEtiquetasMolinos">
            ...
            <p className="separacionTextoM">Ciclón</p>
          </div>
          {loading ? (
