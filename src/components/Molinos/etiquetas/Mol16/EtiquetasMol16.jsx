...
import { apiUrlEtiquetasMol16 } from "../../../../api/molinos/apiMol16";
...
const EtiquetasMol16 = ({ etiquetasMol16, setEtiquetasMol16, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol16);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol16),
          fetchProductos(),
        ]);
        setEtiquetasMol16(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol16.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol16(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol16);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol16", etiquetasMol16);
    }
  }, [etiquetasMol16, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol16, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol16.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol16.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol16.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetasMol16.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
