...
import { apiUrlEtiquetasMol5 } from "../../../../api/molinos/apiMol5";
...
const EtiquetasMol5 = ({ etiquetasMol5, setEtiquetasMol5, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol5);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol5),
          fetchProductos(),
        ]);
        setEtiquetasMol5(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol5.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol5(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol5);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol5", etiquetasMol5);
    }
  }, [etiquetasMol5, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol5, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol5.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol5.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol5.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetasMol5.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
