...
import { apiUrlEtiquetasMol3A } from "../../../../api/molinos/apiMol3A";
...
const EtiquetasMol3A = ({ etiquetasMol3A, setEtiquetasMol3A, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol3A);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol3A),
          fetchProductos(),
        ]);
        setEtiquetasMol3A(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol3A.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol3A(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol3A);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol3A", etiquetasMol3A);
    }
  }, [etiquetasMol3A, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol3A, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol3A.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol3A.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol3A.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetasMol3A.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
