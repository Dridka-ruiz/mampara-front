...
import { apiUrlEtiquetasMol4 } from "../../../../api/molinos/apiMol4";
...
const EtiquetasMol4 = ({ etiquetasMol4, setEtiquetasMol4, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol4);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol4),
          fetchProductos(),
        ]);
        setEtiquetasMol4(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol4.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol4(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol4);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol4", etiquetasMol4);
    }
  }, [etiquetasMol4, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol4, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol4.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol4.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol4.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetasMol4.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
