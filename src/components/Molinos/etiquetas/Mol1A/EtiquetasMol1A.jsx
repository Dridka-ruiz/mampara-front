...
import { apiUrlEtiquetasMol1A } from "../../../../api/molinos/apiMol1A";
...
const EtiquetasMol1A = ({ etiquetasMol1A, setEtiquetasMol1A, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol1A);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      ...
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol1A),
          fetchProductos(),
        ]);
        setEtiquetasMol1A(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol1A.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol1A(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol1A);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol1A", etiquetasMol1A);
    }
  }, [etiquetasMol1A, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol1A, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol1A.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol1A.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol1A.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetasMol1A.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
