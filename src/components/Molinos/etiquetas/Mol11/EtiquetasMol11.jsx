...
import { apiUrlEtiquetasMol11 } from "../../../../api/molinos/apiMol11";
...
const EtiquetasMol11 = ({ etiquetasMol11, setEtiquetasMol11, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol11);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol11),
          fetchProductos(),
        ]);
        setEtiquetasMol11(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol11.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol11(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol11);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol11", etiquetasMol11);
    }
  }, [etiquetasMol11, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol11, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol11.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol11.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetasMol11.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol11.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
