...
import { apiUrlEtiquetasMol9 } from "../../../../api/molinos/apiMol9";
...
const EtiquetasMol9 = ({ etiquetasMol9, setEtiquetasMol9, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol9);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol9),
          fetchProductos(),
        ]);
        setEtiquetasMol9(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol9.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol9(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol9);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol9", etiquetasMol9);
    }
  }, [etiquetasMol9, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol9, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol9.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol9.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol9.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetasMol9.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
