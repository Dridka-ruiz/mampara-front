...
import { apiUrlEtiquetasMol12 } from "../../../../api/molinos/apiMol12";
...
const EtiquetasMol12 = ({ etiquetasMol12, setEtiquetasMol12, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol12);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol12),
          fetchProductos(),
        ]);
        setEtiquetasMol12(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol12.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol12(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol12);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol12", etiquetasMol12);
    }
  }, [etiquetasMol12, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol12, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol12.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol12.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol12.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetasMol12.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
