...
import { apiUrlEtiquetasMol3 } from "../../../../api/molinos/apiMol3";
...
const EtiquetasMol3 = ({ etiquetasMol3, setEtiquetasMol3, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol3);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol3),
          fetchProductos(),
        ]);
        setEtiquetasMol3(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol3.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol3(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol3);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol3", etiquetasMol3);
    }
  }, [etiquetasMol3, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol3, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol3.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol3.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol3.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetasMol3.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
                            ...
                            <div style={{ display: "flex", gap: "7px" }}>
                              ...
                              <div
                                style={{
                                  ...
                                  alignItems: "center",
                                  color:
                                    differenceInDays(item.fecha, new Date()) > 7
