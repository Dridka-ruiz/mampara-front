...
import { apiUrlEtiquetasMol15 } from "../../../../api/molinos/apiMol15";
...
const EtiquetasMol15 = ({ etiquetasMol15, setEtiquetasMol15, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol15);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol15),
          fetchProductos(),
        ]);
        setEtiquetasMol15(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol15.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol15(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol15);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol15", etiquetasMol15);
    }
  }, [etiquetasMol15, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol15, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol15.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol15.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol15.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetasMol15.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
