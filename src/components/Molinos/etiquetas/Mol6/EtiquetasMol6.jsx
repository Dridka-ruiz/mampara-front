...
import { apiUrlEtiquetasMol6 } from "../../../../api/molinos/apiMol6";
...
const EtiquetasMol5 = ({ etiquetasMol6, setEtiquetasMol6, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol6);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      ...
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol6),
          fetchProductos(),
        ]);
        setEtiquetasMol6(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol6.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol6(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol6);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol5", etiquetasMol6);
    }
  }, [etiquetasMol6, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol6, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol6.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol6.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol6.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetasMol6.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
