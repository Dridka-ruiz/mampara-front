...
import { apiUrlEtiquetasMol2 } from "../../../../api/molinos/apiMol2";
...
const EtiquetasMol2 = ({ etiquetasMol2, setEtiquetasMol2, onSaved }) => {
    ...
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol2),
          fetchProductos(),
        ]);
        setEtiquetasMol2(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol2.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol2(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol2);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol2", etiquetasMol2);
    }
  }, [etiquetasMol2, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol2, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol2.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol2.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol2.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
