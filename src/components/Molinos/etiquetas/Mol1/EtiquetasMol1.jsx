...
import { apiUrlEtiquetasMol1 } from "../../../../api/molinos/apiMol1";
...
const EtiquetasMol1 = ({ etiquetasMol1, setEtiquetasMol1, onSaved }) => {
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol1.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol1(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol1);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol1", etiquetasMol1);
    }
  }, [etiquetasMol1, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol1, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol1.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol1.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol1.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
