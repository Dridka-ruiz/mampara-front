...
import { apiUrlEtiquetasMol13 } from "../../../../api/molinos/apiMol13";
...
const EtiquetasMol13 = ({ etiquetasMol13, setEtiquetasMol13, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol13);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol13),
          fetchProductos(),
        ]);
        setEtiquetasMol13(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol13.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol13(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol13);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol13", etiquetasMol13);
    }
  }, [etiquetasMol13, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol13, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol13.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol13.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol13.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetasMol13.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
