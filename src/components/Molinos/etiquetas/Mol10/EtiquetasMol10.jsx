...
import { apiUrlEtiquetasMol10 } from "../../../../api/molinos/apiMol10";
...
const EtiquetasMol10 = ({ etiquetasMol10, setEtiquetasMol10, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol10);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol10),
          fetchProductos(),
        ]);
        setEtiquetasMol10(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol10.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol10(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol10);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol10", etiquetasMol10);
    }
  }, [etiquetasMol10, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol10, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol10.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol10.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol10.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    ...
    setSelectedEtiqueta4(selected2);
    setOpenDialog4(true);
  };
