...
import { apiUrlEtiquetasMol7 } from "../../../../api/molinos/apiMol7";
...
const EtiquetasMol5 = ({ etiquetasMol7, setEtiquetasMol7, onSaved }) => {
  ...
  //console.log("mapeo de datos", etiquetasMol7);
  useEffect(() => {
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasMol7),
          fetchProductos(),
        ]);
        setEtiquetasMol7(etiquetasResponse.data);
  ...
  const handleDeleteEtiqueta = async (etiquetaId) => {
    ...
    const updatedEtiquetas = etiquetasMol7.filter(
      (etiqueta) => etiqueta.id !== etiquetaId
    );
    setEtiquetasMol7(updatedEtiquetas);
    guardarEtiquetas(updatedEtiquetas);
  };
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasMol7);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas Mol5", etiquetasMol7);
    }
  }, [etiquetasMol7, nuevaEtiquetaAgregada]);
  ...
  const guardarEtiquetas = async (etiquetas) => {
    try {
      ...
      await axios.post(apiUrlEtiquetasMol7, etiquetasConExtrusor);
  ...
  const handleEstadoChange = (etiquetaId) => {
    const updatedEtiquetas = etiquetasMol7.map((etiqueta) =>
  ...
  const handleEditEtiqueta = (etiquetaId) => {
    const selected = etiquetasMol7.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleExtrudeEtiqueta = (etiquetaId) => {
    const selected2 = etiquetasMol7.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetasMol7.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
