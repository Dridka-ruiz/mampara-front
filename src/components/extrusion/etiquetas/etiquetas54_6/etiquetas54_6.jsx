...
const EtiquetaTable54_6 = ({ etiquetas54_6, setEtiquetas54_6 }) => {
    ...
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasExt54_6),
          fetchProductos(),
        ]);

        setEtiquetas54_6(etiquetasResponse.data);
  ...
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetas54_6);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas 54_6", etiquetas54_6);
    }
  }, [etiquetas54_6, nuevaEtiquetaAgregada]);
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    // Buscar la etiqueta seleccionada
    const selected4 = etiquetas54_6.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
