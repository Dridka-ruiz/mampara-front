...
const EtiquetaTable54_7 = ({ etiquetas54_7, setEtiquetas54_7 }) => {
    ...
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasExt54_7),
          fetchProductos(),
        ]);

        setEtiquetas54_7(etiquetasResponse.data);
  ...
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetas54_7);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas 54_7", etiquetas54_7);
    }
  }, [etiquetas54_7, nuevaEtiquetaAgregada]);
  ...
  const handleIncompletoEtiqueta = async (etiquetaId) => {
    // Buscar la etiqueta seleccionada
    const selected4 = etiquetas54_7.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
