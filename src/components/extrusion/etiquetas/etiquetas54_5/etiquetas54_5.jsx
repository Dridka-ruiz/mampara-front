...
const EtiquetaTable54_5 = ({ etiquetas54_5, setEtiquetas54_5 }) => {
    ...
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasExt54_5),
          fetchProductos(),
        ]);
        setEtiquetas54_5(etiquetasResponse.data);
  ...
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetas54_5);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas 54 5", etiquetas54_5);
    }
  }, [etiquetas54_5, nuevaEtiquetaAgregada]);
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    // Buscar la etiqueta seleccionada
    const selected4 = etiquetas54_5.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
