...
const EtiquetaTable54_8 = ({ etiquetas54_8, setEtiquetas54_8 }) => {
    ...
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasExt54_8),
          fetchProductos(),
        ]);

        setEtiquetas54_8(etiquetasResponse.data);
  ...
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetas54_8);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas 54_8", etiquetas54_8);
    }
  }, [etiquetas54_8, nuevaEtiquetaAgregada]);
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetas54_8.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
