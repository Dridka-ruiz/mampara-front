...
const EtiquetaTableExt70_3 = ({ etiquetasExt70_3, setEtiquetasExt70_3 }) => {
    ...
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasExt70_3),
          fetchProductos(),
        ]);

        setEtiquetasExt70_3(etiquetasResponse.data);
  ...
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasExt70_3);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas 70_3", etiquetasExt70_3);
    }
  }, [etiquetasExt70_3, nuevaEtiquetaAgregada]);
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    // Buscar la etiqueta seleccionada
    const selected4 = etiquetasExt70_3.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
