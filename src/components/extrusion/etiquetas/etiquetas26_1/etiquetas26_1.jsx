...
const EtiquetaTable26_1 = ({ etiquetas26_1, setEtiquetas26_1 }) => {
    ...
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasExt26_1),
          fetchProductos(),
        ]);

        setEtiquetas26_1(etiquetasResponse.data);
  ...
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetas26_1);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas 26 1", etiquetas26_1);
    }
  }, [etiquetas26_1, nuevaEtiquetaAgregada]);
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    // Buscar la etiqueta seleccionada
    const selected4 = etiquetas26_1.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
