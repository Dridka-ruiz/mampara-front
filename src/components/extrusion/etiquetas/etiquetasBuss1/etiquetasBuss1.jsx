...
const EtiquetaTableBuss1 = ({ etiquetasBuss1, setEtiquetasBuss1 }) => {
    ...
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlBuss1),
          fetchProductos(),
        ]);

        setEtiquetasBuss1(etiquetasResponse.data);
  ...
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasBuss1);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas buss 1", etiquetasBuss1);
    }
  }, [etiquetasBuss1, nuevaEtiquetaAgregada]);
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    // Buscar la etiqueta seleccionada
    const selected4 = etiquetasBuss1.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
