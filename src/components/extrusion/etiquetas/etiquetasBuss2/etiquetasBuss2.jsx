...
const EtiquetaTableBuss2 = ({ etiquetasBuss2, setEtiquetasBuss2 }) => {
    ...
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlBuss2),
          fetchProductos(),
        ]);

        setEtiquetasBuss2(etiquetasResponse.data);
  ...
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasBuss2);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas bus 2", etiquetasBuss2);
    }
  }, [etiquetasBuss2, nuevaEtiquetaAgregada]);
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    // Buscar la etiqueta seleccionada
    const selected4 = etiquetasBuss2.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
