...
const EtiquetaTable54_3 = ({ etiquetas54_3, setEtiquetas54_3 }) => {
    ...
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasExt54_3),
          fetchProductos(),
        ]);

        setEtiquetas54_3(etiquetasResponse.data);
  ...
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetas54_3);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas 54_3", etiquetas54_3);
    }
  }, [etiquetas54_3, nuevaEtiquetaAgregada]);
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    const selected4 = etiquetas54_3.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
