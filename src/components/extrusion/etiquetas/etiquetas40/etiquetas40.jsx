...
const EtiquetaTable40 = ({ etiquetas40, setEtiquetas40 }) => {
    ...
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasExt40),
          fetchProductos(),
        ]);

        setEtiquetas40(etiquetasResponse.data);
  ...
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetas40);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas 40", etiquetas40);
    }
  }, [etiquetas40, nuevaEtiquetaAgregada]);
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    // Buscar la etiqueta seleccionada
    const selected4 = etiquetas40.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
