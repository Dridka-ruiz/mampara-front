...
const EtiquetaTableExt70_1 = ({ etiquetasExt70_1, setEtiquetasExt70_1 }) => {
    ...
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasExt70_1),
          fetchProductos(),
        ]);

        setEtiquetasExt70_1(etiquetasResponse.data);
  ...
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetasExt70_1);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas 70 1", etiquetasExt70_1);
    }
  }, [etiquetasExt70_1, nuevaEtiquetaAgregada]);
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    // Buscar la etiqueta seleccionada
    const selected4 = etiquetasExt70_1.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
