...
const EtiquetaTable54_1 = ({ etiquetas54_1, setEtiquetas54_1 }) => {
    ...
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasExt54_1),
          fetchProductos(),
        ]);

        setEtiquetas54_1(etiquetasResponse.data);
  ...
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetas54_1);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas 54_1", etiquetas54_1);
    }
  }, [etiquetas54_1, nuevaEtiquetaAgregada]);
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    // Buscar la etiqueta seleccionada
    const selected4 = etiquetas54_1.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
