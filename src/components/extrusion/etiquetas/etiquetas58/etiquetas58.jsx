...
const EtiquetaTable58 = ({ etiquetas58, setEtiquetas58 }) => {
    ...
    const cargarEtiquetasDesdeApi = async () => {
      try {
        const [etiquetasResponse, productosResponse] = await Promise.all([
          axios.get(apiUrlEtiquetasExt58),
          fetchProductos(),
        ]);

        setEtiquetas58(etiquetasResponse.data);
  ...
  useEffect(() => {
    if (nuevaEtiquetaAgregada) {
      guardarEtiquetas(etiquetas58);
      setNuevaEtiquetaAgregada(false);
      console.log("etiquetas guardadas 58", etiquetas58);
    }
  }, [etiquetas58, nuevaEtiquetaAgregada]);
  ...
  const handleIncompletoEtiqueta = (etiquetaId) => {
    // Buscar la etiqueta seleccionada
    const selected4 = etiquetas58.find(
      (etiqueta) => etiqueta.id === etiquetaId
    );
