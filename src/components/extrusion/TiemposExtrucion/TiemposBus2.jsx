...
import { fetchEtiquetasBuss2 } from "../../../api/extrusores/apiBuss2";
...
export default function StickyHeadTable() {
    ...
    async function fetchData() {
      try {
        ...
        const ultimoProductoExtruido = productosExtruidos
          .filter((producto) => producto.extrusor === "EXTBUSS-II")
          .pop();
        ...
        const etiquetas = await fetchEtiquetasBuss2();
        ...
        const mappedEtiquetas = etiquetas.map((etiqueta) => {
          ...
          const rendimientoPorHora = producto
            ? (etiqueta.kilos / producto.EXTBUSS_II).toFixed(3)
            : null;

          const rendimientoPorHora300 = producto
            ? (300 / producto.EXTBUSS_II).toFixed(3)
            : null;

          return {
            ...etiqueta,
            rendimiento: producto ? producto.EXTBUSS_II : null,
        ...
        const setUpDatafetchTimeBuss2 = await fetchTimeBuss2();

        const fechasTimeArray = Array.isArray(setUpDatafetchTimeBuss2)
          ? setUpDatafetchTimeBuss2
          : setUpDatafetchTimeBuss2.data;
        ...
        const setUpEXT54II = setUpArray.find(
          (setup) => setup.maquina === "EXTBUSS-II"
        );

        console.log("SetUp for EXTBUSS-II:", setUpEXT54II);

        allProducts.forEach((producto, index, array) => {
          if (setUpEXT54II && sistemasLimpiezaArray.length > 0) {
                  ...
                  if (puntoInicio) {
                    ...
                    if (horaMaximaUltimoIntervalo) {
                      ...
                      if (index < allProducts.length - 1) {
                        ...
                        fetch(`http://localhost:3000/etiquetasBussll/${id}`, {
                          method: "PUT",
                          headers: {
  ...
  return (
    <div>
      <h2>Tiempos Extrusion Buss 2</h2>
