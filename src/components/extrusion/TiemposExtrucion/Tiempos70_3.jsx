...
import { fetchEtiquetas70_3 } from "../../../api/extrusores/apiExt70_3";
    ...
    async function fetchData() {
      try {
        ...
        const ultimoProductoExtruido = productosExtruidos
          .filter((producto) => producto.extrusor === "EXT70-III")
          .pop();
        ...
        const etiquetas = await fetchEtiquetas70_3();
        ...
        const mappedEtiquetas = etiquetas.map((etiqueta) => {
          ...
          const rendimientoPorHora = producto
            ? (etiqueta.kilos / producto.EXT70_III).toFixed(3)
            : null;
          const rendimientoPorHora300 = producto
            ? (300 / producto.EXT70_III).toFixed(3)
            : null;

          return {
            ...etiqueta,
            rendimiento: producto ? producto.EXT70_III : null,
        ...
        const setUpDatafetchTime70_3 = await fetchTime70_3();

        const fechasTimeArray = Array.isArray(setUpDatafetchTime70_3)
          ? setUpDatafetchTime70_3
          : setUpDatafetchTime70_3.data;
        ...
        const setUpEXT70III = setUpArray.find(
          (setup) => setup.maquina === "EXT70-III"
        );

        console.log("SetUp for EXT70-III:", setUpEXT70III);

        allProducts.forEach((producto, index, array) => {
          if (setUpEXT70III && sistemasLimpiezaArray.length > 0) {
            const tiempoLimpieza = setUpEXT70III.tiempos_distancia.find(
              (tiempo) =>
            ...
            if (producto1 && producto2) {
              ...
              if (sistemaCoincidente) {
                ...
                if (sistema2Coincidente) {
                  ...
                  setUpEXT70III.tiempos_distancia.forEach((tiempo) => {
            ...
            if (producto.fecha_real && producto.hora_real) {
              ...
              while (fechaIndex < fechasTimeArray.length) {
                ...
                if (fechaMatch) {
                  ...
                  if (puntoInicio) {
                    console.log(`Punto de inicio encontrado: ${puntoInicio}`);
                    console.log(
                      `Hora de final del intervalo: ${intervaloInicio.horaMaxima}`
                    );
                    console.log(
                      `Hora real dentro del intervalo: ${producto.hora_real}`
                    );

                    // Calcular la diferencia de tiempo entre la hora real y la hora máxima del intervalo
                    ...
                    // Sumar la duración de los intervalos subsiguientes hasta que la duración total sea mayor o igual a tiempo_ext_total
                    ...
                    while (duracionTotalHoras < tiempoExtTotal) {
                      if (siguienteIndice >= intervalos.length) {
                        ...
                        // Actualiza el objeto fechaMatch al cambiar de fecha
                        fechaMatch = fechasTimeArray[fechaIndex];
                        siguienteIndice = 0; // Reiniciar el índice para la nueva fecha
                        console.log(
                          `Cambiando a la siguiente fecha: ${fechasTimeArray[fechaIndex].fecha} (ID: ${fechasTimeArray[fechaIndex].id})` // Mostrar el ID de la fecha
                        );
                        continue;
                      }
                      ...
                      if (
                        ...
                        console.log(
                          `Duración del siguiente intervalo: ${
                            duracionSiguienteIntervalo / 60
                          } horas`
                        );
                        console.log(
                          `Duración total acumulada: ${duracionTotalHoras.toFixed(
                            2
                          )} horas`
                        );
                        console.log(
                          `Hora máxima del último intervalo sumado: ${siguienteIntervalo.horaMaxima}`
                        );
                      } else {
                        console.log(
                    ...
                    // Calcular y mostrar la diferencia entre la duración total acumulada y tiempo_ext_total
                    ...
                    // Calcular la hora total restando la diferencia de la hora máxima del último intervalo
                    if (horaMaximaUltimoIntervalo) {
                      ...
                      // Combinar horaTotalFormateada con fechaUltimoIntervalo
                      const fechaHoraTotalStr = `${fechaUltimoIntervalo} ${horaTotalFormateada}`;
                      ...
                      console.log(
                        `Hora total final ajustada: ${horaTotalFormateada}`
                      );
                      console.log(
                        `Fecha del último intervalo: ${fechaUltimoIntervalo}`
                      );
                      console.log(
                        `Fecha y hora combinadas: ${fechaHoraTotalStr}`
                      );
                    }
                  } else {
                    console.log(
                  ...
                  console.log(`Resultado Final: ${resultadoFormateado}`);
                  ...
                  if (puntoInicio) {
                    ...
                    if (horaMaximaUltimoIntervalo) {
                      ...
                      if (index < allProducts.length - 1) {
                        ...
                        fetch(`http://localhost:3000/etiquetasExt70_3/${id}`, {
                          method: "PUT",
                          headers: {
