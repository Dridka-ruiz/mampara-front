...
export default function StickyHeadTable() {
    ...
    async function fetchData() {
      try {
        ...
        const ultimoProductoExtruido = productosExtruidos
          .filter((producto) => producto.extrusor === "EXTBUSS-I")
          .pop();
        ...
        const etiquetas = await fetchEtiquetasBuss1();
        ...
        const mappedEtiquetas = etiquetas.map((etiqueta) => {
          ...
          const rendimientoPorHora = producto
            ? (etiqueta.kilos / producto.EXTBUSS_I).toFixed(3)
            : null;

          const rendimientoPorHora300 = producto
            ? (300 / producto.EXTBUSS_I).toFixed(3)
            : null;

          return {
            ...etiqueta,
            rendimiento: producto ? producto.EXTBUSS_I : null,
        ...
        const setUpDatafetchTimeBuss1 = await fetchTimeBuss1();

        const fechasTimeArray = Array.isArray(setUpDatafetchTimeBuss1)
          ? setUpDatafetchTimeBuss1
          : setUpDatafetchTimeBuss1.data;
        ...
        const setUpEXT54II = setUpArray.find(
          (setup) => setup.maquina === "EXTBUSS-I"
        );

        console.log("SetUp for EXTBUSS-I:", setUpEXT54II);
                  ...
                  if (puntoInicio) {
                    ...
                    if (horaMaximaUltimoIntervalo) {
                      ...
                      if (index < allProducts.length - 1) {
                        ...
                        fetch(`http://localhost:3000/etiquetasBussl/${id}`, {
                          method: "PUT",
                          headers: {
  ...
  return (
    <div>
      <h2>Tiempos Extrusion Buss 1</h2>
